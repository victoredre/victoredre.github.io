...
Desde configuraciones basicas como procesos de Firmware y/o alta en la WAN para **DHCP, Estatitca o PPPoE** hasta **SD-WAN**, NAT, VPN IPsec, VPN SSL, Políticas de firewall IPv4, Antivirus, Filtrado Web, IPS, App Control, Ruteo estatico y dinámico, BGP en VPN Loopback, segmentación por VLAN's, VDOM, IPS y Web Filter, DNS y VPNE-GRE, así como casos de nivel N2 troubleshooting Va, todo en un solo párrafo, sin listas, listo para tu CV:
Troubleshooting avanzado con análisis de Causa Raíz RCA, troubleshooting via CLI con análisis de flujo de tráfico para validar políticas debug flow, verificación de estado y negociación de túneles VPN IPsec, captura y análisis de paquetes en tiempo real, validación de vecindades y anuncio de rutas BGP, monitoreo de consumo de CPU y memoria para detección de procesos críticos sys top.

**VPN Site-to-Site con Cisco (Interoperabilidad)
Síntoma: FortiGate con Cisco ASA/Router, no levanta.**
...
config vpn ipsec phase2-interface
    edit "to-cisco-p2"
        set phase1name "to-cisco"
        set proposal aes256-sha256
        set dhgrp 14
        set src-subnet <tu-red>
        set dst-subnet <red-cisco>
    next
end
```
